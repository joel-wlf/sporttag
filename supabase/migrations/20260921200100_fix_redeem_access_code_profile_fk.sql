...
-- `devices.registered_by` referenziert `profiles(id)`, aber anonyme
-- Stationssitzungen haben nie ein Profil (profiles ist ausschließlich für
-- Organisatoren, siehe docs/datenkonzept.md Abschnitt 8). `redeem_access_code`
-- setzte `registered_by` bisher unbedingt auf `auth.uid()` und verletzte damit
-- bei jedem echten anonymen Codebeitritt den Fremdschlüssel. Gefunden beim
-- SQL-Testlauf der Station-Laufzeit-Migration.
...
begin
  ...
  -- Nur setzen, wenn die aktuelle Identität tatsächlich ein Organisatorprofil
  -- hat; eine anonyme Stationssitzung lässt das Feld NULL.
